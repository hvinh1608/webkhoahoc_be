...
class TracNghiemController extends Controller
{
    ...
    {
        $data = TracNghiem::where('khoa_hoc_id', $khoaHocId)
            ->where('tinh_trang', 1)
            ->select('id', 'cau_hoi', 'dap_an_1', 'dap_an_2', 'dap_an_3', 'dap_an_4')
            ->inRandomOrder()
            ->get();
    ...
    public function nopBaiTN(Request $request)
    {
        $user = Auth::guard('sanctum')->user();
        $so_cau_dung = 0;
        $khoa_hoc_id = $request->khoa_hoc_id; // FE cần truyền lên
        ...
        // Lưu kết quả
        KetQuaTracNghiem::updateOrCreate(
            [
                'khach_hang_id' => $user->id,
                'khoa_hoc_id' => $khoa_hoc_id,
                'loai' => 'final', // hoặc loại khác nếu có
            ],
            [
                'so_cau_dung' => $so_cau_dung,
            ]
