...
class BaiHocController extends Controller
{
    ...
    public function getdata()
    {
        ...
        $data = BaiHoc::join('loai_khoa_hocs', 'id_khoa_hoc', 'loai_khoa_hocs.id')
            ->select('bai_hocs.*', 'loai_khoa_hocs.ten_khoa_hoc')
            ->get();
        return response()->json([
            'data'    =>  $data,
        ]);
    ...
    public function search(Request $request)
    {
        ...
        $data = BaiHoc::join('loai_khoa_hocs', 'bai_hocs.id_khoa_hoc', '=', 'loai_khoa_hocs.id')
            ->select('bai_hocs.*', 'loai_khoa_hocs.ten_khoa_hoc')
            ->where(function ($query) use ($noi_dung) {
                $query->where('bai_hocs.tieu_de', 'like', "%$noi_dung%")
                    ->orWhere('loai_khoa_hocs.ten_khoa_hoc', 'like', "%$noi_dung%");
            })
            ->get();
        return response()->json([
            'data' => $data
